...
import tomllib
import argparse
from titanoboa_cli.logging import logger, set_log_level
import sys
from titanoboa_cli.constants import CONFIG_NAME


BOA_CLI_VERSION_STRING= "Titanoboa CLI v{}"
...
""",
    ...
    # run_parser.add_argument("--no-fork", action="store_true", help="Path to the script you want to run.")
    # dry run or something
    ...
    if args.command:
        importlib.import_module(f"titanoboa_cli.commands.{args.command}").main(args)
    else:
        main_parser.print_help()
    return 0
...
def get_version() -> int:
    with open(
        ...
        return BOA_CLI_VERSION_STRING.format(boa_cli_data["project"]["version"])
